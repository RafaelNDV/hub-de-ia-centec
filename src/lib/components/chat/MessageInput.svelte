				...
				<div class="">
					...
					>
						...
						<div
							...
							class="centec-chat-composer flex-1 flex flex-col relative w-full shadow-lg rounded-3xl border {$temporaryChatEnabled
								...
								<div class="ml-1 self-end flex items-center flex-1 min-w-0">
									...
									>
										<button
											...
											class="centec-chat-action centec-chat-action-accent bg-transparent hover:bg-gray-100 text-gray-700 dark:text-white dark:hover:bg-gray-800 rounded-full size-[1.875rem] flex justify-center items-center outline-hidden focus:outline-hidden shrink-0"
											...
											{#if $_user?.role === 'admin' || ($_user?.permissions?.chat?.stt ?? true)}
												...
												<Tooltip content={$i18n.t('Dictate')}>
													<button
														...
														class="centec-chat-action text-gray-600 dark:text-gray-300 hover:text-gray-700 dark:hover:text-gray-200 transition rounded-full p-1.5 self-center mr-0.5"
														type="button"
														on:click={async () => {
															try {
										...
										{#if prompt === '' && files.length === 0 && ($_user?.role === 'admin' || ($_user?.permissions?.chat?.call ?? true))}
											<div class=" flex items-center">
												...
												<Tooltip content={$i18n.t('Voice mode')}>
													<button
														class="centec-chat-primary-action bg-black text-white hover:bg-gray-900 dark:bg-white dark:text-black dark:hover:bg-gray-100 transition rounded-full p-[5px] self-center"
														type="button"
														on:click={async () => {
											...
											<div class=" flex items-center">
												...
												>
													<button
														...
														class="centec-chat-primary-action {!(
